                         ...
                         <?php if($this->session->userdata('tipo_user') == 'Administrador' || $this->session->userdata('tipo_user') == 'Direccion'){ ?>
                            <li class="treeview active">
                                ...
                                <ul class="treeview-menu">
                                    ...
                                    <li class="active"><a href="<?php echo base_url(); ?>reporteador/getUsers" ><i class="fa fa-angle-double-right"></i> Usuarios</a></li>
                ...
                <section class="content">
					...
					<div class="box">
						<div class="box-header">
							<h3 class="box-title">LISTA DE USUARIOS <?php echo $cityCode; ?></h3>                                    
						</div><!-- /.box-header -->
						<div class="box-body table-responsive">
                            <section id="no-more-tables">
    							<table id="UsersTable" class="table table-striped table-condensed cf" style="margin:0 auto 0;">
    								<thead class="widget-header">
    									<tr>
    										<th style="text-align:center;" class="col-md-1" >TIPO</th>
    								...
    								while ($fila = mysql_fetch_array($usuarios_ciudad)){
    									...
    									if(mb_strlen($userId) == 1){
    										$userId = "000".$userId;
    									}else if(mb_strlen($userId) == 2){
    										$userId = "00".$userId;
    									}else if(mb_strlen($userId) == 3){
    										$userId = "0".$userId;
    									}
    								...
    								?>
    									<tr>
    										<td  data-title='TIPO' class="col-md-1"><?php echo $fila['emp_user_type']; ?></th>
    										<td  data-title='CLAVE' class="col-md-1" ><?php echo $clave; ?></td>
    										<td  data-title='RAZON SOCIAL' class="col-md-3" ><?php echo $fila['emp_razon_social']; ?></td>
    										<td  data-title='DIRECCIÓN' style='text-align:center;'>:P</td>
    										<td  data-title='RFC' style='text-align:left;'><?php echo $fila['emp_RFC'];?></td>
    										<td  data-title='TELEFONO' style='text-align:right; font-size: 11px;'><?php echo $fila['emp_telefono'];?></td>
    										<td  data-title='CORREO' class="col-md-1" ><?php echo $fila['emp_correo'];?></td>
    										<td  data-title='OPCIONES' class="col-sm-2"  style="text-align: center;">
    											<button class='btn btn-primary btn-sm btnMatriz' data-toggle="modal" title="Enviar mensaje" data-target="#compose-modal" correo="<?php echo $fila['emp_correo'];?>" idmio='<?php echo $fila['emp_id']; ?>'>
    												<i class='fa fa-envelope'></i>
    											</button>
                                                <?php if(($fila['emp_rol_usuario'] == 2) && ($this->session->userdata('tipo_user') != "Direccion") ){

                                                }else{
                                                    echo '<a role="button" class="btn btn-danger btn-sm deleteusers" title="Desactivar usuario" idUser="'.$fila["emp_id"].'" >
                                                            <i class="fa fa-trash-o"></i>
                                                        </a>';
                                                }

                                                if($fila['emp_tienda'] == 1){ ?>
                                                        <a href="#" role="button" class='btn btn-danger btn-sm Eliminartienda' id="Desactivaruser_<?php echo $fila['emp_id']; ?>" idactivar="Activaruser_<?php echo $fila['emp_id']; ?>" title="Desactivar tienda" data-toggle="modal" idUser="<?php echo $fila['emp_id']; ?>" >
                                                            <i class='fa fa-credit-card'></i>
                                                        </a>
                                                        <a href="#" role="button" class='btn btn-success btn-sm Agregartienda' style="display:none;" id="Activaruser_<?php echo $fila['emp_id']; ?>" idactivar="Desactivaruser_<?php echo $fila['emp_id']; ?>" data-toggle="modal" title="Activar tienda" idUser="<?php echo $fila['emp_id']; ?>" >
                                                            <i class='fa fa-credit-card'></i>
                                                        </a>
                                                <?php }else{ ?>
                                                        <a href="#" role="button" class='btn btn-danger btn-sm Eliminartienda' style="display:none;" id="Desactivaruser_<?php echo $fila['emp_id']; ?>" idactivar="Activaruser_<?php echo $fila['emp_id']; ?>" title="Desactivar tienda" data-toggle="modal" idUser="<?php echo $fila['emp_id']; ?>" >
                                                            <i class='fa fa-credit-card'></i>
                                                        </a>
                                                        <a href="#" role="button" class='btn btn-success btn-sm Agregartienda' id="Activaruser_<?php echo $fila['emp_id']; ?>" idactivar="Desactivaruser_<?php echo $fila['emp_id']; ?>" data-toggle="modal" title="Activar tienda" idUser="<?php echo $fila['emp_id']; ?>" >
                                                            <i class='fa fa-credit-card'></i>
                                                        </a>
                                                <?php } ?>
    										</td>
			...
			$(function(){
               ...
               $(".btnMatriz").click(function(){
                ...
                $(".Agregartienda").click(function(){
                    ...
                    var x = $("#" + $(this).attr('idactivar'));
                    var activo = $(this);
                    $.ajax({
                        type : 'GET',
                        url  : '<?php echo base_url();?>users/activarTienda/'+id,
                ...
                $(".Eliminartienda").click(function(){
                    ...
                    $.ajax({
                        type : 'GET',
                        url  : '<?php echo base_url();?>users/desactivarTienda/'+id,
                        success: function(data){
                            if(data === 'ACTIVO'){
                                activo.hide();
                                x.show();
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            if(data === 'noborrado'){
                                ...
                                window.location.href = "<?php echo base_url();?>reporteador/getUsers"
                            }
                        }
                    });
                });
