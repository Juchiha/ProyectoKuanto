...
<html>
	...
	<!-- fin del Header -->
    								...
    								<table id="ListaInsumos" class="table table-striped table-condensed cf" style="margin:0 auto 0;">
    									<thead class="widget-header">
    										<tr>
                                                ...
                                                <th style="text-align:center;" class="col-sm-1">CREADOR</th>
    									...
    									while ($fila = mysql_fetch_array($prospectos)){?>		
    										<tr>
                                                ...
                                                <td  data-title='CREADOR' style='text-align:left;' class="col-sm-1"><?php echo $fila['admin']; ?></td>
        ...
        <div class="modal fade" id="email-modal-total" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    ...
                    <form id="enviomensaje" method ="POST">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <div class="input-group">
                                    ...
                                    <select name="ciudades" id="ciudades" class="form-control">
                                        ...
                                        <?php while($fil = mysql_fetch_array($ciudades)){ ?>
                                            <option values="<?php echo $fil['city_id'];?>"><?php echo $fil['city_nombre']; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="input-group">
                                    <span class="input-group-addon">Asunto:</span>
                                    <input name="subject" type="text" id="textsubject2" class="form-control" placeholder="Asunto">
                                </div>
                            </div>
                            <div class="form-group">
                                <textarea name="mensaje" id="email_message2" class="form-control" placeholder="Mensage" style="height: 120px;"></textarea>
                            </div>
                        </div>
                        <div class="modal-footer clearfix">
                            ...
                            <button type="button" id="envioMensaje23" class="btn btn-primary pull-left"><i class="fa fa-envelope"></i> Enviar Mensaje</button>
                        </div>
                    </form>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
		...
		$(function(){
            $('#ListaInsumos').dataTable({
                ...
                "aoColumns": [{ "bSortable": false },null, { "bSortable": false },  { "bSortable": false }, { "bSortable": false },{ "bSortable": false }, null, null, { "bSortable": false }] ,
            ...
            $("#envioMensaje2").click(function(){
                ...
                if($("#textsubject").val().length < 1){
                    ...
                    $.ajax({
                        url : "<?php echo base_url(); ?>mensajes/EnvioMensajesProspectosIndividualEfren",
                        type:"POST",
                        data:{ email_cc: $("#textsubject").val(), message: $("#email_messageNojoda").val(), email_to: $("#correoSky").val()},
                        success:function(data){
                            $("#textsubject").val("");
                            $("#email_messageNojoda").val("");
                            $("#correoSky").val("0");
            ...
            $("#envioMensaje23").click(function(){
                if($("#textsubject2").val().length < 1){
                    alertify.error("Estas seguro de enviar este correo sin un asunto!");
                    $("#textsubject2").focus();
                }else if($("#email_message2").val().length < 1){
                    alertify.error("Estas seguro de enviar este correo sin un mensaje!");
                    $("#email_message2").focus();
                }else{
                    $.ajax({
                        url : "<?php echo base_url(); ?>prospectos/envioMaxivoEfren",
                        type:"POST",
                        data:{ email_cc: $("#textsubject2").val(), message: $("#email_message2").val(), ciudad : $("#ciudades").val()},
                        success:function(data){
                            $("#textsubject").val("");
                            $("#email_message").val("");
                            $("#ciudades").val("0");
